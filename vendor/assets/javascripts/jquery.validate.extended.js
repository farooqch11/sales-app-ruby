...
$.validator.addMethod(
    ...
    function(value, element) {
        $.ajax({
            type: "GET",
            url: "/account/check_domain_availability",
            data: "account[domain]="+value,
            dataType:"html",
            async: false,
            success: function(msg)
            {
                g = msg;
                //If domain exists, set response to true
                if( $.parseJSON(msg)[0] == true){
                    response = false;
                    message =  $.parseJSON(msg)[1]
                }
                if( $.parseJSON(msg)[0] == false){
                    response = true;
                }
            }
        });
...
$.validator.addMethod(
    ...
    function(value, element) {
        $.ajax({
            type: "GET",
            url: "/distributors/referral_email_exists",
            data: "email="+value,
            dataType:"html",
            async: false,
            success: function(msg)
...
$.validator.addMethod(
    "requiredSelect",
    function(value, element) {
        if($(element).find(":selected").text() == '' && value == ''){
            return false;
        }
        return true;
    },
